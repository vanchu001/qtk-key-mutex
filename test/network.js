...
describe('#network', function () {
    before(async function () {
        let server = new Locker.NetworkServer({
            host: "127.0.0.1",
            port: 9999,
    ...
    it('[promise.all and client reconnect]', async function () {
        this.timeout(100000000);
        setTimeout(() => {
            mutex._locker._client._close();
        }, 1000);
    ...
    it('[promise.all and one of timeout]', async function () {
        ...
        assert(l1 instanceof Error && l2 == 'func2' && endAt - startAt < 2000, `promise.all and one of server timeout`);
    });
})
